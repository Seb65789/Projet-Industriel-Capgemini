...
def eye_aspect_ratio(eye):
    A = np.linalg.norm(np.array(eval(eye.iloc[1])) - np.array(eval(eye.iloc[5])))
    B = np.linalg.norm(np.array(eval(eye.iloc[2])) - np.array(eval(eye.iloc[4])))
    C = np.linalg.norm(np.array(eval(eye.iloc[0])) - np.array(eval(eye.iloc[3])))
    ear = (A + B) / (2.0 * C)
    return ear
...
for video_name in video_names:
    ...
    for frame in range(len(df)):
        eye_coordinates_left = df.loc[frame, ['left_eye_263','left_eye_387','left_eye_385','left_eye_326','left_eye_380','left_eye_373']]
        eye_coordinates_right = df.loc[frame, ['right_eye_33','right_eye_160','right_eye_158','right_eye_133','right_eye_153','right_eye_144']]
