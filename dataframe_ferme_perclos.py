...
for x in range(876) :
    if x >= 30:
        df_EAR_MAR[f'ferme_left_{x}'] = df_EAR_MAR[f'ferme_left_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
        df_EAR_MAR[f'ferme_right_{x}'] = df_EAR_MAR[f'ferme_right_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
        df_EAR_MAR[f'ferme_mean_{x}'] = df_EAR_MAR[f'ferme_mean_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
    else:
        df_EAR_MAR[f'ferme_left_{x}'] = df_EAR_MAR[f'ferme_left_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
        df_EAR_MAR[f'ferme_right_{x}'] = df_EAR_MAR[f'ferme_right_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
        df_EAR_MAR[f'ferme_mean_{x}'] = df_EAR_MAR[f'ferme_mean_{x}'].apply(lambda valeur: True if valeur < 0.25 else False)
