...
def calculs_signes(list_points):
    # les 8*3+1 premières valeurs sont pour les coordonnées de l'oeil droit
...
for ligne in range(df_coordinates.shape[0]):
    ...
    for i in range(1, df_coordinates.shape[1], 3):
        ...
        point_coordinates = [df_coordinates.iloc[ligne, i], df_coordinates.iloc[ligne, i + 1],
                              df_coordinates.iloc[ligne, i + 2]]
    ...
    # Créer une liste avec le nom de la vidéo, les coordonnées, les résultats des signes et la classe
    row_data = [video_and_coordinates[0]] + results

    df.loc[len(df)] = row_data

# Extraire la classe de la colonne "nom_video"
classe_column = df_coordinates["nom_video"].str.extract(r'(\d)')
# Concaténer la colonne "classe" à la fin du DataFrame
df = pd.concat([df, classe_column], axis=1)
# Renommer la colonne
